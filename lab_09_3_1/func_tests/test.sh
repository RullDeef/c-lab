...
do
  ...
  then
    status=$(drmemory.exe -batch -ignore_kernel -no_use_stderr -- ./app.exe $args 1>./func_tests/out/pos_${n}_out.txt 2>./func_tests/out/pos_${n}_err.txt)
  elif command -v valgrind &> /dev/null
  then
    status=$(valgrind -q -- ./app.exe $args 1>./func_tests/out/pos_${n}_out.txt 2>./func_tests/out/pos_${n}_err.txt)
  else
    status=$(./app.exe $args 1>./func_tests/out/pos_${n}_out.txt 2>./func_tests/out/pos_${n}_err.txt)
  fi
